...
def get_btcprice(currency):
    ...
    price_dollars = parse_float(response_json["data"]["1"]["quotes"][currency]["price"])
    ...
    if currency is not None:
        if currency not in ["CAD", "USD"]:
            return build_validation_result(
                False,
                "currency",
                "Sorry, I'm just able to convert from CAD or USD for now.",
            )
...
def convert_dollars(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        validation_result = validate_data(birthday, dollars, currency, intent_request)
